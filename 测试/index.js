  ...
  setTimeout(() => {
    let daohangye = new DataV({
      ...
      ComList: {
        ...
        "生产调度模块-船舶轮播列表": {
          ...
          data: {
            ...
            filters: function (res) { // 过滤器
              try {
                return res.data.rows.map(i => {
                  i.dependPlantime = i.dependPlantime != null ? i.dependPlantime.slice(5) : ""
        ...
        "核心指标模块-隐藏按钮": { id: "main-img_9IWeR", type: "图片", event: { "originClickName": "hxzbmk_hideBtn" } },
          ...
          data: {
            ...
            filters: function (res) { // 过滤器
              try {
                ...
                //  return res.data.filter(i => i.type == '十大船旗').map((i,index) => {
                // i.id = index;
                // i.label =(index+1) + " " + i.nationName + ":" + i.shipNum + "艘次"
                // return i
                //  }).slice(0,5)
          ...
          data: {
            filters: function (res) { // 过滤器
              try {
                ...
                // return res.data.filter(i => i.type == '十大船旗').map((i,index) => {
                // 	i.id = index;
                //   i.label = (index+5) + " "  + i.nationName + ":" + i.shipNum + "艘次"
                //   return i
                // }).slice(5,10)
              } catch (e) {
          ...
          data: {
            filters: function (res) { // 过滤器
              try {
                ...
                return arr.slice(0, 5)
                // return res.data.filter(i => i.type == '十大航运').map((i,index) => {
                //   i.id = index;
                //   i.label = (index+1) + " "  + i.nationName + ":" + i.shipNum + "艘次"
                //   return i
                // }).slice(0,5)
        ...
        "在港重点物资运输模块-当年重点运输船舶列表": {
          ...
          data: {
            ...
            filters: function (res) { // 过滤器
              try {
                return res.data.rows.map(i => {
                  i.hwlCnt = parseFloat(Math.floor(i.hwlCnt / 10000)).toLocaleString() + "万"
                  return i
                })
              } catch (e) {
            ...
            filters: function (res) { // 过滤器
              try {
                ...
                return res.data.rows.map(i => {
                  ...
                  i.arrangeLineTime = i.arrangeLineTime != null ? i.arrangeLineTime.slice(5) : ""
                  i.zuoyejieshushijian = i.zuoyejieshushijian != null ? i.zuoyejieshushijian.slice(5) : ""
                  i.pilots = i.pilots || "自引"
                  i.seaRoute = seaRouteConfig[i.seaRoute];
            ...
            filters: function (res) {
              try {
                ...
                return res.data.rows.map(i => {
                  ...
                  i.leavePlantime != null ? i.leavePlantime.slice(5) : ""
      ...
      ComEvent: {
          ...
          if (name) {
            ...
            this.ComApi['getGgmjrtgl'].call(this, { type: this.Store.module_hddtqk.havenIdList[name] }).then(res => {
              try {
                ...
                stage.get(this.ComList['航道动态情况模块-航门通过量下钻弹窗-区域翻牌器'].id).render(res.data.rows.map(i => {
                  i.s = 'Beijing AQI';
                  i.y = i.cnt;
                  i.x = i.time;
                  return i
            ...
            try {
              ...
              if (dataList.length == 0) {
                ...
                dom.render(dataList.map(i => {
                  i.jhjcgsj = `抛锚:${i.predictAnchorTime != null ? i.predictAnchorTime.slice(5) : ""}<br>起锚:${i.predictMoveAnchorTime != null ? i.predictMoveAnchorTime.slice(5) : ""}`
                  return i
                }).slice(0, 100))
              }
          ...
          this.ComApi['Getjjhscbqdlb'].call(this, {}).then(res => {
            try {
              stage.get(this.ComList['交通效率指标模块-经济航速列表弹窗-船舶列表'].id).render(res.data.rows.map(i => {
                i.preArriveTime = i.preArriveTime != null ? i.preArriveTime.slice(4) : ""
                return i
              }).slice(0, 100))
            } catch (e) {
          ...
          switch (e[0].id) {
            ...
            case 300:
              ...
              this.ComApi['getTsmShipList'].call(this, {}).then(res => {
                try {
                  stage.get(this.ComList['航道动态情况模块-虾改条的船舶清单弹窗-船舶列表'].id).render(res.data.rows);
                  stage.get(this.ComList['航道动态情况模块-虾改条的船舶清单弹窗-标题'].id).render([{ value: `今日虾改条清单（共${res.data.rows.length}艘）` }]);
                } catch (e) {
        ...
        mddtmk_mdxzqctc: function () {
          ...
          this.ComApi['Getjzlzjsc_jbsy'].call(this, { dateType: "年" }).then(res => {
            try {
              stage.get(this.ComList['锚地动态情况模块-保税油下钻弹窗-历年保税油加注船舶列表'].id).render(res.data.rows.filter(i => i.gyl).map((i, index) => {
                i.time = i.time + "年"
      ...
      ComApi: {
        ...
        getShipInfoByPolygonOrgGet: function (params) {
          return new Promise((resolve, reject) => this.request("/bas/vel/ads/getShipInfoByPolygonOrgGet", params).then(res => resolve(res)))
        },
        // 新气泡数据
